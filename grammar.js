...
module.exports = grammar({
  ...
  rules: {
    ...
    _if_actions_end: $ => seq($._left_delimiter, 'end', $._right_delimiter),
    ...
    chained_pipeline: $ => prec.left(PREC.primary, seq($._pipeline, '|', $._pipeline)),

    parenthesized_pipeline: $ => seq('(', $._pipeline_or_variable, ')'),
    ...
    variable: $ => seq('$', optional(field('name', alias(
        token(repeat1(choice(letter, unicodeDigit))),
        $.identifier,
      )),
    )),

    identifier: _ => token(seq(letter, repeat(choice(letter, unicodeDigit)))),
